...
export function ExternalAuthenticationButtons() {
    ...
    useEffect(() => {
        function resetRedirectingProvider() {
            setRedirectingProvider(null);
        }

        window.addEventListener('pageshow', resetRedirectingProvider);

        return () => window.removeEventListener('pageshow', resetRedirectingProvider);
    }, []);
    ...
    return (
        <Stack spacing={1.5}>
            <Button
                fullWidth
                loading={redirectingProvider === 'Google'}
                loadingPosition="start"
                onClick={() => beginExternalLogin('Google')}
                startIcon={<GoogleLogo width={22} height={22} />}
                variant="outlined"
            >
            ...
            <Button
                fullWidth
                loading={redirectingProvider === 'Microsoft'}
                loadingPosition="start"
                onClick={() => beginExternalLogin('Microsoft')}
                startIcon={
                    <img 
                        src="/images/microsoft-logo.svg.svg"
                        width={20}
                        height={20} 
                    />
                }
...
export function GoogleLogo({ width, height }: { width: number, height: number }) {
    return (
        <svg
            width={width}
            height={height}
